...
class UserController {
  ...
  try {
    ...
    const { skip = skip1 , limit = limit1 , sort } = req.query;
    if ((Object as any).values(JSON.parse(sort))[0] === '' ||
    Object.keys(JSON.parse(sort))[0] !== 'name' ||
    Object.keys(JSON.parse(sort))[0] !== 'email' || Object.keys(JSON.parse(sort))[0] === '') {
      next ({ error: 'Error Occurred in Sorting', status });
  }
    console.log('Skip is', skip, 'Limit is: ', limit, sort);
    const role = {role: 'trainee'};
    const userRepository = new UserRepository();
    const data = { role, skip , limit, sort };
